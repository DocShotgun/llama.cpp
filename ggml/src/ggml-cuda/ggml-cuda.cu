...
// one entry in the order in which MoE expert tensors are copied to the device
struct ExpsReadaheadEntry {
    const void * ptr;
    size_t size;
};

// double-buffered pinned host cache used to hide the slow remote-NUMA source copy
struct ExpsReadaheadCache {
    ...
    bool grow(size_t sz, std::unique_lock<std::mutex> & lock) {
        ...
        // wait for in-flight fills so they never touch a buffer being freed
        for (int s = 0; s < 2; ++s) {
...
// thread pool that prefetches expert tensors into the pinned cache
...
static bool exps_readahead_copy(int device, exps_readahead_role role, const char * tname, ggml_tensor * tensor, const void * data, size_t size) {
    ...
    // grow or lazily allocate the pinned buffer on first use
    if (c->slot[0] == nullptr || size > c->buf_size) {
        size_t need = std::max(size, c->buf_size);
        if (c->buf_size == 0) {
            if (!c->alloc(need)) {
                GGML_LOG_WARN("%s: failed to allocate %.2f MiB of pinned memory for exps readahead (dev %d, role %s); falling back to slow copy\n",
            ...
            if (!c->grow(size, lock)) {
                GGML_LOG_WARN("%s: failed to grow pinned buffer to %.2f MiB (dev %d, role %s); falling back to slow copy\n",
    ...
    // record the copy order so the next tensor in the sequence can be prefetched
    ...
    if (hit_slot >= 0) {
        // already staged in pinned host memory: fast copy, then free the slot
        ...
        // not staged: fall back to the direct (slow) copy
    ...
    // pick the next tensor in the sequence and stage it into the other slot
    ...
    // split the copy across the pool so each NUMA node touches its own shard
...
static void ggml_backend_cuda_buffer_set_tensor(ggml_backend_buffer_t buffer, ggml_tensor * tensor, const void * data, size_t offset, size_t size) {
    ...
    // route full MoE tensor copies through the pinned readahead cache
